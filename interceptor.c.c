...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	...
	if (current_uid() != 0)
	{
		if (cmd == REQUEST_SYSCALL_INTERCEPT || cmd == REQUEST_SYSCALL_RELEASE){
			...
			//More pid validity checking
			if (pid == 0 || check_pid_from_list(current->pid, pid)==-EPERM){
				return -EPERM;
			}
		}
	}
	
	//Syscall validity checking
	...
	//Actually do the given commands
	if (cmd == REQUEST_SYSCALL_INTERCEPT)
	{
		...
		//Save the original function for interceptor
	...
	else if (cmd == REQUEST_SYSCALL_RELEASE)
	{
		...
		//Remove all monitored pids for the syscall
		spin_lock(&pidlist_lock);
	...
	else if (cmd == REQUEST_START_MONITORING)
	{
		...
		if (pid != 0)
		{
			//Add pid to syscalls list of monitored pid
	...
	else if (cmd == REQUEST_STOP_MONITORING)
	{
		...
		if (table[syscall].intercepted == 0 || (!check_pid_monitored(syscall,pid) && pid != 0)){
			return -EINVAL;
		}
...
static int init_function(void) {
    ...
    while (current_syscall < NR_syscalls){
        INIT_LIST_HEAD(&(table[current_syscall].my_list));
        table[current_syscall].intercepted = 0;
        table[current_syscall].monitored = 0;
        table[current_syscall].listcount = 0;
        current_syscall++;
    }
